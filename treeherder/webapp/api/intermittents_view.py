...
class Failures(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # queryset = Bugscache.objects.filter(job__push__time__range=(startday, endday), job__repository__name__in=tree).select_related()
...
class FailuresByBug(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = BugJobMap.objects.select_related('job').filter(bug_id=bug_id, job__repository__name__in=tree,
...
class FailureCount(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        push_query = Push.objects.filter(repository__name__in=tree, time__range=(startday, endday))\
             .select_related('repository').annotate(date=TruncDate('time')).values('date')\
             .annotate(test_runs=Count('author')).order_by('date').values('date', 'test_runs')

        if bug_id:
        #TODO bug_id will need to be changed to bug__id
            ...
            queryset = Job.objects.filter(push__time__range=(startday, endday), repository__name__in=tree,
                                          failure_classification__id=4).select_related('repository', 'push',
                                          'failure_classification').annotate(date=TruncDate('push__time'))\
